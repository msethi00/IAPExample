...
{	
    BOOL isOk = [self isTransactionAndItsReceiptValidWithError:outError];
    if (!isOk)
    {
    ...
    NSString *payload = [NSString stringWithFormat:@"{\"receipt-data\" : \"%@\", \"password\" : \"%@\"}",
                         jsonObjectString, self.controller.itcContentProviderSharedSecret];

    NSData *payloadData = [payload dataUsingEncoding:NSUTF8StringEncoding];
    
    NSLog(@"RRTransactionVerifier INFO isSandboxTransaction bit set to %d",self.isSandboxTransaction);
...
- (BOOL)isTransactionAndItsReceiptValidWithError:(out NSError **)outError;
{
    ...
    if (![self doTransactionDetailsMatchPurchaseInfo:purchaseInfoDict])
    {
        return NO;
    }
...
- (BOOL)doTransactionDetailsMatchPurchaseInfo:(NSDictionary *)purchaseInfoDict
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString*) receiptString error:(out NSError **)outError
{
    ...
    NSDictionary *purchaseInfoFromTransaction = self.originalPurchaseInfoDict;
    ...
    // Do additional time checks for the transaction and receipt. (this comment per Apple)
...
static BOOL checkReceiptSecurity(NSString *purchase_info_string, NSString *signature_string, CFDateRef purchaseDate)
{
	...
	NSData *purchase_info_data, *signature_data;
    NSArray *anchors;
	...
	purchase_info_data = base64_decode(purchase_info_string);
	size_t purchase_info_length = [purchase_info_data length];
	const uint8_t *purchase_info_bytes = [purchase_info_data bytes];


	signature_data = base64_decode(signature_string);
	size_t signature_length = [signature_data length];
    const uint8_t *signature_bytes = [signature_data bytes];
